import React from "react";
import { Component } from "react";
import {Table, Button,Message,Icon} from 'semantic-ui-react';
import web3 from "../ethereum/web3";
import Campaign from "../ethereum/campaign";
import { Router} from '../routes.js';



class RequestRow extends Component{
    constructor(props) {
        super(props);
      
      }

    onApprove= async() =>{
        try{       
            const campaign = Campaign(this.props.address);
            const accounts = await web3.eth.getAccounts();
            this.props.data.changeHidden(false,'Waiting for approval,this will take about 15-30 seconds');
            this.props.data.errorMessage(true);
            await campaign.methods.approveRequest(this.props.id).send({
                from: accounts[0]
            });
            this.props.data.changeHidden(true);
            Router.pushRoute(`/campaigns/${this.props.address}/requests`);
        }catch(err){
            this.props.data.changeHidden(true);
            this.props.data.errorMessage(false,err.message);
            console.log(err);
        }

    };

    onFinalize = async() =>{
        try{
            const campaign = Campaign(this.props.address);
            const accounts = await web3.eth.getAccounts();
            this.props.data.changeHidden(false,'Waiting for finalization,this will take about 15-30 seconds');
            this.props.data.errorMessage(true);
            await campaign.methods.finalizeRequest(this.props.id).send({
                from: accounts[0]
            });
            this.props.data.changeHidden(true);
            Router.pushRoute(`/campaigns/${this.props.address}/requests`);
        }catch(err){
            this.props.data.changeHidden(true);
            this.props.data.errorMessage(false,err.message);
        }

    }

    render(){
        const {Row,Cell}=Table;
        const readyToFinalize = this.props.request.approvalCount > this.props.approversCount / 2;
        return(
            <Row disabled={this.props.request.complete} positive={readyToFinalize && !this.props.request.complete}>
                <Cell>{this.props.id}</Cell>
                <Cell>{this.props.request.description}</Cell>
                <Cell>{web3.utils.fromWei(this.props.request.value, 'ether')}</Cell>
                <Cell>{this.props.request.recipient}</Cell>
                <Cell>{this.props.request.approvalCount}/{this.props.approversCount}</Cell>
                <Cell>
                    {this.props.request.complete ? null : (
                    <Button color ='green' basic onClick={this.onApprove}>Approve</Button>
                    )}
                </Cell>
                <Cell>
                    {this.props.request.complete ? null : (
                    <Button color = 'teal' basic onClick={this.onFinalize}>Finalize</Button>
                    )}
                </Cell>

              

            </Row>

            
            
        )
    }
}

export default RequestRow;
